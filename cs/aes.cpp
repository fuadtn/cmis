...
int aes(int argc, char argv[][128])
{
	...
	{
		cout << endl << "AES-128 /4-4-10/" << endl;
		cout << "-e - encryption;" << endl;
		cout << "-d - decryption;" << endl;
		cout << "example: ./aes -e in.dat key.dat" << endl << endl;
		return 0;
	}
	if (argc < 4)
	{
	...
	if(strlen(key) < 16)
	{
		cout << "error: wrong key length" << endl;
		return -1;
	}
	...
	{
		input[length] = 1;
		for (int i = length + 1; i < length + 16; i++)
		input[i] = 0;
	...
	for (int k = 0; k < length / 16; k++)
	{
		...
		for (int i = 0; i < Nb; i++)
		for (int j = 0; j < Nb; j++)
		c_output[i + 4 * j + 16 * k] = state[i][j];
	}


	f_write.open(argv[2], ios_base::binary);
	for (int i = 0; i < length; i++)
	f_write << (char) c_output[i];
	f_write.close();
	return 0;
}
